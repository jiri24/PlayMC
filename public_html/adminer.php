...
<tr><th>SystÃ©m<td>',html_select("auth[driver]",$Vb,DRIVER),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
sprintf('Pro pÅ™ihlÃ¡Å¡enÃ­ k SQLite implementujte metodu %s.','login()');return
true;}function
tableName($Pg){return
h($Pg["Name"]);}function
...
',(support("comment")?"<td".($rb?"":" class='hidden'").">".'KomentÃ¡Å™':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.5' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($rb?"":" class='hidden'")."><input name='fields[$s][comment]' value='".h($o["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.5' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.2.5' alt='^' title='".'PÅ™esunout nahoru'."'>&nbsp;"."<input type='image' class='icon' name='down[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.2.5' alt='v' title='".'PÅ™esunout dolÅ¯'."'>&nbsp;":""),($Xe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$s]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.2.5' alt='x' title='".'Odebrat'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
<th>NÃ¡zev
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.2.5' alt='+' title='PÅ™idat dalÅ¡Ã­'></noscript>&nbsp;
</thead>
<?php
...
<input type="submit" value="UloÅ¾it">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.5' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Kd=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$Kd++;}queries_redirect(ME."processlist=",lang(array('Byl ukonÄen %d proces.','Byly ukonÄeny %d procesy.','Bylo ukonÄeno %d procesÅ¯.'),$Kd),$Kd||!$_POST["kill"]);}page_header('Seznam procesÅ¯',$n);echo'
<form action="" method="post">
